...
function NativeTabsView(props) {
    ...
    const children = routes
        .filter(({ key }) => (0, utils_1.shouldTabBeVisible)(descriptors[key].options))
        .map((route) => {
        return (<react_tabs_1.TabsContent key={route.name} value={route.name} className={native_tabs_styles_1.default.tabContent} forceMount>
          {descriptors[route.key].render()}
        </react_tabs_1.TabsContent>);
    });
    return (<react_tabs_1.Tabs className={native_tabs_styles_1.default.nativeTabsContainer} defaultValue={defaultTabName} value={value} onValueChange={(value) => {
            navigation.dispatch({
                type: 'JUMP_TO',
                target: state.key,
                payload: {
      ...
      <react_tabs_1.TabsList aria-label="Main" className={native_tabs_styles_1.default.navigationMenuRoot}>
        {items}
...
function TabItem(props) {
    ...
    return (<react_tabs_1.TabsTrigger value={route.name} className={native_tabs_styles_1.default.navigationMenuTrigger}>
